...
namespace Drupal\wxt\Generators;
...
/**
 * Implements a WxT sub-profile generator for Drush.
 *
...
final class SubProfileGenerator extends BaseGenerator {
   ...
   * {@inheritdoc}
   */
  protected $name = 'wxt-subprofile';

  /**
   * {@inheritdoc}
   */
  protected $description = 'Generates a WxT Subprofile.';
  ...
  protected function interact(InputInterface $input, OutputInterface $output) {
    ...
    if ($io->confirm('Do you want to exclude any components of WxT?', FALSE)) {
      $filter = function ($name) {
        return strpos($name, 'wxt_') === 0;
      };
      ...
      $questions['exclude'] = new ChoiceQuestion('WxT components to exclude (optional), entered as keys separated by commas (e.g. wxt_media, wxt_layout)', $modules);
